# Matter and Edge AI add-on for nRF Connect SDK

The Matter and Edge AI add-on for the nRF Connect SDK provides support for developing applications that combine the Matter protocol with Edge AI solutions.

## Getting started

To get started with Matter and Edge AI add-on for nRF Connect SDK, follow [documentation](https://nrfconnectdocs.nordicsemi.com/addons/ncs-matter-edge-ai/latest/index.html).

##  License

Source code included within this repository is licensed under the [LicenseRef-Nordic-5-Clause](https://github.com/nrfconnect/ncs-matter-edge-ai/blob/main/LICENSE)
